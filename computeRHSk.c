...
void computeRHSk(GRID *g,SOLN *s,double *l2rho, int myid)
{
  //
  int i,j,k,m,f,n;
  int f1,f2;
    ...
    for(j=0;j<chainSize;j++)
	 {
	   ...
	   else
	   {
        ...
        else if(rightCell == -5)
        {
          ...
          else if(j==nghost-1 || j==chainSize-(nghost))
          {
            if(g->idup[icell]==2)
            {
	   ...
	   if(fabs(g->f[j][0]) < 1.e-14)
	   	printf("Error because of zero flux\n");
	   else if (g->f[j][0] != g->f[j][0]) 
	   	printf("Error because of NaN\n");
